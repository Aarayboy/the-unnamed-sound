    ...
    , ui(new Ui::MainWindow),username(_username)
{
    ...
    qInfo()<<musicList;
    qWarning()<<musicList.size()<<picList.size()<<lyricList.size();
...
void MainWindow::on_next_clicked() // 下一首
{
    ...
    mediaPlayer->setSource(playList[(curPlayIndex-1)%sumSong]);
    mediaPlayer->setSource(playList[curPlayIndex]);
...
void MainWindow::write_diary_slot()
{
    con = new content();
    con->show();
    connect(con, &content::close_con_signals, this, &MainWindow::close_con_slot);
}

void MainWindow::close_diary_slot()
{
    delete cho;cho = nullptr;
}

void MainWindow::close_con_slot()
{
    delete con;con = nullptr;
}

void MainWindow::backsee_diary_slot()
{
    bac = new backsee();
    bac->init();
    bac->show();
...
void MainWindow::on_pushButton_clicked()
{
    ...
    cho->put_picture();
    cho->show();
    connect(cho, &choose::write_signals, this, &MainWindow::write_diary_slot);
    connect(cho, &choose::backsee_signals, this, &MainWindow::backsee_diary_slot);
    connect(cho, &choose::close_signals, this, &MainWindow::close_diary_slot);
}
